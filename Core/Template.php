...
class Template
{
    ...
    private function _parser($template = null, $data)
    {
        ...
        if ($data) {
            foreach ($data as $key => $val) {
                $parse = is_array($val) ? $this->_parseMatch($key, $val, $template) : $this->_parse($key, (string) $val, $template);
    ...
    private function _parse($key, $val, $string)
    {
     ...
     * @return array
     */
    private function _parseMatch($var, $data, $string)
    {
        $replace = [];
